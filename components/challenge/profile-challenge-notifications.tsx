...
export default function ProfileChallengeNotifications() {
  ...
  const loadChallenges = async () => {
    ...
    try {
      ...
      addDebug(`Found ${data?.length || 0} raw challenges`);
      ...
      for (const challenge of data || []) {
        try {
          ...
          challengesWithProfiles.push({
            ...challenge,
            challenger: profile || { username: "Unknown", full_name: "Unknown User" },
          });
  ...
  const acceptChallenge = async (challengeId: string, category: string, difficulty: string, opponentName: string) => {
    try {
      ...
      router.push(`/quiz?category=${category}&difficulty=${difficulty}&challenge=${challengeId}&questions=10&opponentName=${encodeURIComponent(opponentName)}&challengerTurn=false`);
    } catch (error: any) {
      toast({
        title: "Error",
  ...
  return (
    <Card className={cardClasses}>
      <CardHeader className={hasActiveChallenges ? "bg-red-100 dark:bg-red-900 p-4" : "p-4"}>
        <CardTitle className="flex items-center justify-between">
          <div className={`flex items-center gap-2 ${hasActiveChallenges ? "text-red-800 dark:text-red-200" : "text-orange-800 dark:text-orange-200"}`}>
            <Bell className={`h-5 w-5 ${hasActiveChallenges ? "animate-bounce" : ""}`} />
            {hasActiveChallenges ? (
              <span className="font-bold">URGENT: {challenges.length} Challenge{challenges.length !== 1 ? "s" : ""} Pending!</span>
            ) : (
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                  <div className="flex items-center space-x-3">
                    <Avatar className="h-10 w-10">
                      ...
                      <AvatarFallback className="bg-green-100 text-green-700">{getUserInitials(challenge.challenger)}</AvatarFallback>
                    </Avatar>
                    <div>
                      <p className="font-medium">
                        <span className="text-green-600 dark:text-green-400">{challenge.challenger.full_name || challenge.challenger.username}</span> challenged you!
                      </p>
                      <div className="flex flex-wrap items-center gap-2 text-sm text-gray-500">
                        <Badge variant="secondary" className="text-xs">{categoryLabels[challenge.category] || challenge.category}</Badge>
                  ...
                  <div className="flex gap-2 justify-end">
                    ...
                    <Button
                      size="sm"
                      onClick={() => acceptChallenge(challenge.id, challenge.category, challenge.difficulty, challenge.challenger.full_name || challenge.challenger.username)}
