...
// Pull categories from the data layer so UI always matches the synced data folder
import { getAllCategories } from "@/data/categories"
import type { QuizCategory } from "@/types/quiz"

// Map known category ids to icons. Unknown ids fall back to HelpCircle.
const iconMap: Record<string, React.ReactNode> = {
  ...
  "islamic-finance": <BriefcaseMedical className="h-8 w-8 text-green-600 dark:text-green-400" />,
...
function getIconFor(categoryId: string) {
  return iconMap[categoryId] ?? <HelpCircle className="h-8 w-8 text-green-600 dark:text-green-400" />
}
...
export default function CategoriesPage() {
  ...
  // Load categories from data source once on mount
  const categories: QuizCategory[] = useMemo(() => {
    try {
      const list = getAllCategories()
      // Sort by title for a consistent UX
  ...
  return (
    <main className="min-h-screen w-full bg-gradient-to-b from-green-50 to-green-100 dark:from-green-950 dark:to-green-900">
      <div className="mx-auto max-w-5xl px-4 py-8">
        <Card className="w-full border-green-200 shadow-lg dark:border-green-800">
          ...
          <CardContent>
            {categories.length === 0 ? (
              <div className="text-center text-sm text-muted-foreground">No categories found.</div>
            ) : (
